...
def test_no_pressing_needs_returns_idle():
    ...
    result = evaluate_mortal_action(_mortal(cs), _state(), 0)
    assert result == "idle"
...
def test_fatigue_blocks_action():
    ...
    result = evaluate_mortal_action(_mortal(cs, fatigue=0.9), _state(), 0)
    assert result == "idle"
...
def test_spend_at_spend_location():
    ...
    result = evaluate_mortal_action(_mortal(cs, kb, loc_id=spend_loc_id), _state(), 0)
    assert result == "spend"
...
def test_leisure_action_when_pressing_at_pop():
    cs = MortalAgentState(needs=[_pressing("leisure")])
    result = evaluate_mortal_action(
        _mortal_with_pop(cs),
        _state_with_pop(),
        0,
...
def test_leisure_skipped_when_gain_below_decay_rate():
    ...
    result = evaluate_mortal_action(_mortal_with_pop(cs), _state_with_pop(), 0)
    assert result == "idle"


def test_socialize_action_when_pressing_at_pop():
    cs = MortalAgentState(needs=[_pressing("belonging")])
    result = evaluate_mortal_action(
        _mortal_with_pop(cs),
        _state_with_pop(),
        0,
...
def test_leisure_skipped_without_pop_in_state():
    ...
    result = evaluate_mortal_action(_mortal_with_pop(cs), s, 0)
    assert result == "idle"
...
def test_leisure_uses_pop_milieu_over_pop_id():
    ...
    result = evaluate_mortal_action(mortal, s, 0)
    assert result == "leisure"
...
def test_might_as_well_roll_succeeds_triggers_collect():
    ...
    state.pops = {}
    with patch("logic.mortal_agent_logic.random.random", return_value=0.0):  # always succeeds
...
def test_might_as_well_roll_fails_returns_idle():
    ...
    state.locations = {}
    state.pops = {}
    with patch("logic.mortal_agent_logic.random.random", return_value=1.0):  # always fails
        result = evaluate_mortal_action(mortal, state, 0)
    assert result == "idle"
...
def test_might_as_well_culture_boosts_prob():
    ...
    from logic.mortal_agent_logic import _might_as_well_prob, MIGHT_AS_WELL_BASE_PROB
...
def test_might_as_well_prob_clamped():
    ...
    from logic.mortal_agent_logic import _might_as_well_prob
...
from logic.mortal_agent_logic import _cosine_sim, _pop_social_quality
...
from logic.mortal_agent_logic import _effective_commerce_quality
...
def test_sell_blocked_when_skill_system_engaged_without_trade():
    ...
    result = evaluate_mortal_action(mortal, _state(), 0)
    assert result != "sell"
