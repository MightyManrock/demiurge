...
- **Vessel crew pops are exempt from both mechanics.** `Pop.asset_crew_for` is set on any pop that serves as crew for a vessel asset. Skip these in `_check_pop_splinters` and `_check_pop_reabsorption` with `if pop.asset_crew_for is not None: continue`. Crew composition is a separate future concern (hiring from a Common:transport pop, etc.) and should not be disrupted by passive world simulation.
